...
extension SimScenarioExt on SimScenario {
  String get title {
    switch (this) {
      case SimScenario.healthy:
...
class SimulatorService {
  ...
  static List<LogPoint> generateFullRun(SimScenario scenario) {
    ...
    for (int i = 0; i < totalPoints; i++) {
      ...
      if (tps > 80) {
        ...
        if (scenario == SimScenario.lazyO2Sensor) {
      ...
      double dpfDp = 0.0;
      if (scenario == SimScenario.dpfBlockedUnderboost) {
        dpfDp = 28.0 + (rpm - 1800) / 4900.0 * 18.0;
      } else if (scenario == SimScenario.dpfEgrDelete) {
        dpfDp = 0.1; // Stała nienaturalna wartość (zawieszona)
      } else {
        dpfDp = 3.2 + (rpm / 6000.0) * 8.5;
      }

      double dpfSoot = (scenario == SimScenario.dpfBlockedUnderboost) ? 91.0 : 28.0;
      if (scenario == SimScenario.dpfEgrDelete) dpfSoot = 0.0;

      double egt = (scenario == SimScenario.dpfBlockedUnderboost)
      ...
      double egrCmd = 0.0;
      if (scenario != SimScenario.dpfEgrDelete) {
      ...
      points.add(LogPoint(
        ...
        values: {
          ...
          "STFT": double.parse(stft.toStringAsFixed(1)),
          "LTFT": (scenario == SimScenario.skodaRapidInjector) ? -14.5 : 2.3,
          ...
          "MIS_1": 0.0,
          "MIS_2": 0.0,
          "MIS_3": (rpm >= 3500 && (scenario == SimScenario.cylinderMisfireFuel || scenario == SimScenario.cylinderMisfireSpark)) ? (5.0 + rnd.nextDouble() * 10).roundToDouble() : 0.0,
          "MIS_4": 0.0,
        },
      ));
    }
  ...
  /// Rozpoczyna płynną symulację na żywo z odpytywaniem 20 Hz
  void startLivePull({
    required SimScenario scenario,
    required void Function(LogPoint point) onPoint,
    ...
    _timer = Timer.periodic(const Duration(milliseconds: 50), (timer) {
      ...
      _elapsedMs += 50.0;
      final progress = min(1.0, _elapsedMs / 5500.0);
      _currentRpm = 1800.0 + (6700.0 - 1800.0) * pow(progress, 0.95);

      final tps = (progress > 0.05 && progress < 0.95) ? 100.0 : 0.0;

      double targetBoost = 0.0;
      if (_currentRpm < 2200) {
        targetBoost = 0.1 + (_currentRpm - 1800) / 400 * 0.45;
      } else if (_currentRpm < 3200) {
        targetBoost = 0.55 + (_currentRpm - 2200) / 1000 * 0.80;
      } else {
        targetBoost = 1.35 - (_currentRpm - 3200) / 3500 * 0.15;
      }

      double boost = 0.0;
      if (_currentRpm < 2200) {
        boost = 0.1 + (_currentRpm - 1800) / 400 * 0.4;
      } else if (_currentRpm < 3200) {
        boost = 0.5 + (_currentRpm - 2200) / 1000 * 0.85;
      } else {
        boost = 1.35 - (_currentRpm - 3200) / 3500 * 0.15;
      }

      double ign = 8.0 + (_currentRpm - 1800) / 4900 * 9.5;
      double afr = 14.7;
      if (tps > 80) afr = 11.8;
      double maf = 18.0 + (_currentRpm - 1800) / 4900 * 195.0;

      // Scenariusze
      if (scenario == SimScenario.boostLeak && _currentRpm >= 4150) {
        boost -= 0.85;
      } else if (scenario == SimScenario.knockRetard && _currentRpm >= 4100 && _currentRpm <= 5400) {
        ign -= 6.5;
      } else if (scenario == SimScenario.leanAfr && _currentRpm >= 4200) {
        afr = 14.3;
      } else if (scenario == SimScenario.badMaf && _currentRpm >= 4200) {
        maf = 120.0;
      }

      boost += (_rnd.nextDouble() - 0.5) * 0.02;
      targetBoost += (_rnd.nextDouble() - 0.5) * 0.01;
      ign += (_rnd.nextDouble() - 0.5) * 0.3;

      final point = LogPoint(
        timeMs: _elapsedMs,
        values: {
          "RPM": double.parse(_currentRpm.toStringAsFixed(0)),
          "BOOST": double.parse(boost.toStringAsFixed(2)),
          "TARGET_BOOST": double.parse(targetBoost.toStringAsFixed(2)),
          "MAF": double.parse(maf.toStringAsFixed(1)),
          "IGN": double.parse(ign.toStringAsFixed(1)),
          ...
          "IAT": 28.0,
          "ECT": 90.0,
          "SPEED": double.parse((50.0 + progress * 90.0).toStringAsFixed(0)),
        },
      );
